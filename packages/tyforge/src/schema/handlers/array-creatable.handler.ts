...
export class ArrayCreatableHandler implements IFieldHandler {
  execute(
    ...
    const validate = resolveCreatableMethod(field.creatable, field, shouldUseCreateMethod(field, useAssign));
    const items: unknown[] = [];
    for (let index = 0; index < value.length; index++) {
      const item = value[index];
      const itemPath = `${field.path}[${index}]`;
      if (field.required && TypeGuard.isNullish(item)) return err(requiredError(itemPath));
      const result = validate(item, itemPath);
