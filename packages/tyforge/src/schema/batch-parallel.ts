...
class ParallelBatchProcessor implements IParallelProcessor {
  async process<TSchema extends ISchema>(
    ...
    if (isTs) {
      const distWorkerPath = nodePath.resolve(
        currentDir,
        "..",
        "..",
        "dist",
    ...
    const timeout = Math.max(
      1000,
      Math.min(options.workerTimeout ?? 30000, 300000),
    );
        ...
        try {
          const workerDone = new Promise<IWorkerResult>((resolve, reject) => {
            ...
            timer = setTimeout(() => {
              ...
              worker.terminate();
              reject(
                new Error("Worker timed out after " + workerTimeout + "ms"),
              );
            }, workerTimeout);
          });
  ...
  private reconstructResults<TSchema extends ISchema>(
    ...
    for (const result of workerResults) {
      ...
      for (const failure of result.failures) {
        const detailResult = TypeGuard.isString(
