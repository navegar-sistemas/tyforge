...
describe("SchemaBuilder + DomainEvent", () => {
  it("Aggregate emite domain event na criação", () => {
    ...
    assert.equal(events[0].eventName.getValue(), "user.created");
  });
  ...
  it("domain event tem id, occurredAt e version", () => {
    ...
    const event = result.value.getDomainEvents()[0];
    assert.ok(event.id);
    assert.ok(event.occurredAt.getValue() instanceof Date);
    assert.equal(event.version.getValue(), "1.0.0");
  });
...
import { FPassword } from "@tyforge/type-fields/security/password.typefield";
