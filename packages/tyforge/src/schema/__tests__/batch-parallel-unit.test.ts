...
function makeMixed(length: number, invalidEvery: number) {
  return Array.from({ length }, (_, i) =>
    i % invalidEvery === 0 ? makeInvalid(i) : makeValid(i),
  );
}
...
describe("batchCreate — parallel", () => {
  it("validates items with concurrency 2", async () => {
    ...
    const result = await resolveBatch(
      validator.batchCreate(items, { concurrency: 2, chunkSize: 10 }),
    );
  ...
  it("matches sequential results for same data", async () => {
    ...
    const parallel = await resolveBatch(
      validator.batchCreate(items, { concurrency: 2, chunkSize: 25 }),
    );
  ...
  it("falls back to sequential when concurrency is 1", async () => {
    ...
    const result = await resolveBatch(
      validator.batchCreate(items, { concurrency: 1 }),
    );
...
describe("ParallelBatchProcessor", () => {
  ...
  const assignSimple: TAssignFn<typeof simpleSchema> = (data) =>
    validator.assign<unknown>(data);
  const nestedValidator = SchemaBuilder.compile(nestedSchema);
  const assignNested: TAssignFn<typeof nestedSchema> = (data) =>
    nestedValidator.assign<unknown>(data);

  before(() => {
    const created = createParallelProcessor();
    assert.ok(
      created,
      "ParallelBatchProcessor not available in this environment",
    );
    processor = created;
  });
  ...
  describe("nested schema", () => {
    ...
    it("reports errors in nested fields", async () => {
      const items = [
        { title: "Item1", active: true, address: { street: "", city: "SP" } },
      ];
      const result = await processor.process(
        nestedSchema,
        items,
