...
export class NoNumericSeparatorRule extends Rule {
  ...
  constructor(severity: "error" | "warning" = "error") {
    super("no-numeric-separator", "Numeric separators are forbidden. Use 30000 instead of 30_000", severity);
  }
  ...
  check(line: string, lineNumber: number, filePath: string): IRuleViolation | null {
    ...
    if (this.pattern.test(code)) {
      return this.violation(lineNumber, filePath, "Numeric separators are forbidden. Use 30000 instead of 30_000");
    }
