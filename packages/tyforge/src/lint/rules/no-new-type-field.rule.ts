...
export class NoNewTypeFieldRule extends Rule {
  ...
  constructor(severity: "error" | "warning" = "error") {
    super("no-new-type-field", "Forbids instantiating TypeFields with new — use create() or createOrThrow()", severity);
  }
  ...
  check(line: string, lineNumber: number, filePath: string): IRuleViolation | null {
    ...
    if (filePath.includes("format_vo.ts") || filePath.includes("type-field")) return null;

    return this.violation(lineNumber, filePath, "Don't use 'new' for TypeFields — use create() or createOrThrow()");
  }
}
