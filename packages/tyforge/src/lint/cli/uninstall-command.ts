...
export class UninstallCommand {
  async execute(): Promise<void> {
    ...
    if (!ToolDetect.isTTY()) {
      console.error("tyforge-lint --uninstall requires an interactive terminal (TTY).");
      return;
    }
  ...
  private async run(prompt: ToolPrompt, cwd: string): Promise<void> {
    ...
    if (hookManagerType) {
      ...
      if (manager.isInstalled(cwd)) {
        ...
        prompt.printInfo("No tyforge-lint hook found installed.");
      }
    } else {
    ...
    if (fs.existsSync(configPath)) {
      const removeConfig = await prompt.confirm("Remove tyforge-lint.config.json?", false);
      if (removeConfig) {
        try {
          fs.unlinkSync(configPath);
          prompt.printSuccess("Config file removed.");
        } catch {
          prompt.printInfo("Could not remove tyforge-lint.config.json");
        }
      } else {
