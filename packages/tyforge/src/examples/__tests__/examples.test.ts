...
import {
  FString, FEmail, FInt, FBoolean, FId, FAppStatus, FHttpStatus, FPageNumber, FPageSize,
...
describe("07 — Aggregate + DomainEvent", () => {
  ...
  class EventOrderTest extends DomainEvent<TEventPayload> {
    protected readonly _classInfo = { name: "EventOrderTest", version: "1.0.0", description: "Test order event" };
    readonly queueName = FString.createOrThrow("test-events");
  ...
  it("cria Aggregate com domain event", () => {
    ...
    assert.equal(events[0].eventName.getValue(), "order.created");
  });
...
describe("14 — Repository + Paginação", () => {
  ...
  class TestRepo extends RepositoryCrud<TestUser> {
    ...
    async findAll(params?: IPaginationParams): ResultPromise<Paginated<TestUser>, Exceptions> {
      ...
      const pageSize = params?.pageSize.getValue() ?? all.length;
      ...
      const paginated = Paginated.create<unknown, TestUser>({
        items,
        totalItems: all.length,
        page: params?.page.getValue() ?? 1,
        pageSize: params?.pageSize.getValue() ?? all.length,
      });
      return paginated;
    }
  ...
  it("findAll sem paginação retorna todos", async () => {
    ...
    assert.equal(all.value.totalItems.getValue(), 5);
  });
  ...
  it("findAll com paginação", async () => {
    ...
    assert.equal(page1.value.totalPages.getValue(), 3);
    assert.equal(page1.value.page.getValue(), 1);
  });
