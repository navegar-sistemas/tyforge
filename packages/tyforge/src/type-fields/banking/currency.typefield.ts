...
import { FMoney, TMoney } from "./money.typefield";
...
export class FCurrency extends FMoney {
  ...
  static validateType(
    value: unknown,
    fieldPath = "Currency",
  ): Result<TCurrency, ExceptionValidation> {
    return TypeGuard.extractNumber(value, fieldPath);
  }

  // Accepts decimal input, converts to integer cents internally
  static override create<T = TCurrency>(
    raw: T,
    fieldPath = "Currency",
  ): Result<FCurrency, ExceptionValidation> {
    const typed = FCurrency.validateType(raw, fieldPath);
   ...
   */
  static override assign<T = TMoney>(
    value: T,
    fieldPath = "Currency",
  ): Result<FCurrency, ExceptionValidation> {
    const typed = FCurrency.validateType(value, fieldPath);
    if (isFailure(typed)) return err(typed.error);
  ...
  static override zero(fieldPath = "Currency"): FCurrency {
    return FCurrency.createOrThrow(0, fieldPath);
  }
  ...
  override getDescription(): string {
    return (
      "Decimal currency input that stores as " +
      "integer cents internally. Accepts decimal " +
      "values (10.50), converts to cents (1050). " +
      "Inherits arithmetic and comparisons " +
      "from FMoney."
    );
  }
