...
export class FEmvQrCodePayload extends TypeField<TEmvQrCodePayload, TEmvQrCodePayloadFormatted> {
  ...
  static validateType(value: unknown, fieldPath: string): Result<TEmvQrCodePayload, ExceptionValidation> {
    return TypeGuard.isString(value, fieldPath);
  }

  static create<T = TEmvQrCodePayload>(raw: T, fieldPath = "EmvQrCodePayload"): Result<FEmvQrCodePayload, ExceptionValidation> {
  ...
  static createOrThrow(raw: TEmvQrCodePayload, fieldPath = "EmvQrCodePayload"): FEmvQrCodePayload {
    const result = this.create(raw, fieldPath);
    if (isFailure(result)) throw result.error;
    return result.value;
  }

  static assign<T = TEmvQrCodePayload>(value: T, fieldPath = "EmvQrCodePayload"): Result<FEmvQrCodePayload, ExceptionValidation> {
  ...
  override getDescription(): string {
    return "EMV QR Code payload — encoded string containing payment data (merchant, amount, currency) for instant payment processing.";
  }
