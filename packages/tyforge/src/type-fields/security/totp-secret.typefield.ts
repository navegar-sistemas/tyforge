...
export class FTotpSecret extends TypeField<TTotpSecret, TTotpSecretFormatted> {
    ...
    if (!FTotpSecret.BASE32_REGEX.test(value)) {
      return err(
        ExceptionValidation.create(
          fieldPath,
          "TOTP secret must be a valid base32-encoded string",
        ),
      );
    }
    return OK_TRUE;
  ...
  static validateType(
    value: unknown,
    fieldPath: string,
  ): Result<TTotpSecret, ExceptionValidation> {
    return TypeGuard.isString(value, fieldPath);
  }

  static create<T = TTotpSecret>(
    raw: T,
  ...
  override getDescription(): string {
    return (
      "TOTP shared secret encoded in base32" + " for two-factor authentication."
    );
  }
