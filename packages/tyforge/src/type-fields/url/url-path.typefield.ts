...
// Detects directory traversal (../) in any
// position, including backslash variants
...
export class FUrlPath extends TypeField<TUrlPath, TUrlPathFormatted> {
  ...
  static validateType(
    value: unknown,
    fieldPath = "UrlPath",
  ): Result<TUrlPath, ExceptionValidation> {
  ...
  protected override validateRules(
    ...
    if (ABSOLUTE_URL_REGEX.test(value)) {
      return err(
        ExceptionValidation.create(
          fieldPath,
          "Path must be relative, not an absolute URL.",
        ),
      );
    }
    ...
    if (CRLF_REGEX.test(value)) {
      return err(
        ExceptionValidation.create(
          fieldPath,
          "Path must not contain CRLF characters.",
        ),
      );
    }
  ...
  static create<T = TUrlPath>(
    raw: T,
    fieldPath = "UrlPath",
  ): Result<FUrlPath, ExceptionValidation> {
  ...
  override formatted(): TUrlPathFormatted {
  ...
  override getDescription(): string {
    return (
      "Relative URL path. No traversal," +
      " CRLF, null bytes," +
      " or absolute URLs."
    );
  }
