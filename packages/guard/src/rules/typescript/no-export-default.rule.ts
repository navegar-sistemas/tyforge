...
export class NoExportDefaultRule extends Rule {
  ...
  constructor(severity: "error" | "warning" = "error") {
    super("no-export-default", "Forbids export default — use named exports", severity, true);
  }
  ...
  check(line: string, lineNumber: number, filePath: string): IRuleViolation | null {
    ...
    for (const pattern of this.patterns) {
      if (pattern.test(code)) {
        return this.violation(lineNumber, filePath, "Export default is forbidden — use named export");
      }
    }
    return null;
  }
