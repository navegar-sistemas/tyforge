...
export class InitCommand {
  async execute(): Promise<void> {
    ...
    if (!ToolDetect.isGitRepo(cwd)) {
      console.error("tyforge-guard --init requires a git repository.");
      return;
    }
  ...
  private async run(prompt: ToolPrompt, cwd: string): Promise<void> {
    ...
    actions.push("Write tyforge-guard.config.json");
    ...
    prompt.printSuccess("Config written to tyforge-guard.config.json");

    if (enableHook && hookManager) {
      const manager = HookManagerFactory.create(hookManager);
      const result = manager.setup("npx tyforge-guard --staged", cwd);
