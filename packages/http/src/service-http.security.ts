...
export class ServiceHttpSecurity {
  ...
  static buildUrl(baseUrl: FUrlOrigin, endpoint: FUrlPath): Result<FUrlFull, Exceptions> {
    try {
      const safeBase = baseUrl.getValue().endsWith("/") ? baseUrl.getValue() : `${baseUrl.getValue()}/`;
      const cleanPath = endpoint.getValue().startsWith("/") ? endpoint.getValue().substring(1) : endpoint.getValue();
      return ok(FUrlFull.createOrThrow(new URL(cleanPath, safeBase).toString(), "url"));
    } catch {
