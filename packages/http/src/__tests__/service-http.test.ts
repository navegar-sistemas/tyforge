...
function fint(value: number): FInt {
  return FInt.createOrThrow(value);
}

function fformat(value: "json" | "form"): FHttpFormat {
  return FHttpFormat.createOrThrow(value);
}

function fbool(value: boolean): FBoolean {
  return FBoolean.createOrThrow(value);
}
...
class TestServiceHttp extends ServiceHttp {
  ...
  testPut<D = unknown>(endpoint: FUrlPath, data: D, options?: TRequestOptions) {
...
describe("ServiceHttp — request execution", () => {
  ...
  it("POST sends JSON body to correct URL", async () => {
    const result = await service.testPost(fpath("users"), { name: "Maria" });
    assertSuccess(result);
    assert.equal(result.value.status.getValue(), 200);
  ...
  it("PUT sends JSON body", async () => {
    const result = await service.testPut(fpath("users/1"), { name: "Updated" });
    assertSuccess(result);
...
describe("ServiceHttp — response parsing", () => {
  ...
  it("returns error for non-ok status with externalError details", async () => {
    ...
    const result = await service.testPost(fpath("users"), { name: "" });
...
describe("ServiceHttp — authentication", () => {
  ...
  it("does not include auth headers when authenticated is false", async () => {
    const result = await service.testGet(fpath("public"));
    assertSuccess(result);
...
describe("ServiceHttp — security", () => {
  ...
  it("rejects endpoint with path traversal at TypeField level", () => {
    assert.ok(isFailure(FUrlPath.create("../admin")));
  });

  it("rejects endpoint with absolute URL at TypeField level", () => {
    assert.ok(isFailure(FUrlPath.create("https://evil.com/api")));
  });

  it("rejects endpoint with CRLF injection at TypeField level", () => {
  ...
  it("rejects array values in GET query params", async () => {
    const result = await service.testGet(fpath("users"), { ids: [1, 2, 3] });
    assertFailure(result);
    assert.equal(result.error.code, "FAILED_SERIALIZATION");
    assert.equal(mockCalls.length, 0);
  });
...
describe("ServiceHttp — timeout", () => {
  ...
  it("passes AbortController signal to fetch when timeout is set", async () => {
    ...
    await service.testGet(fpath("users"), undefined, { timeout: fint(5000) });
  ...
  it("does not pass signal when timeout is not set", async () => {
    ...
    await service.testGet(fpath("users"));
...
describe("ServiceHttp — edge cases", () => {
  ...
  it("GET with non-record data skips query params silently", async () => {
    const result = await service.testGet(fpath("users"), "not-a-record");
    assertSuccess(result);
    // Non-record data is ignored for GET, URL has no query params
    assert.equal(mockCalls[0].url, "https://api.test.com/users");
  });

  it("returns FAILED_URL_CONSTRUCTION for invalid endpoint", () => {
    // FUrlOrigin rejects invalid URLs at creation time
    assert.ok(isFailure(FUrlOrigin.create("not-a-valid-url")));
  });
});
