...
describe("Linter inline disable comments", () => {
  ...
  const tmpFile = path.join(tmpDir, `tyforge-lint-test-${Date.now()}.ts`);
  ...
  it("disable-next-line disables all rules", () => {
    const violations = lintContent("// tyforge-lint-disable-next-line\nconst x: any = value as string;");
    assert.equal(violations.length, 0);
  });

  it("disable-next-line with rule name disables only that rule", () => {
    const violations = lintContent("// tyforge-lint-disable-next-line no-any\nconst x: any = value as string;");
    assert.equal(violations.length, 1);
    assert.equal(violations[0].rule, "no-cast");
  });

  it("disable-line disables all rules on the same line", () => {
    const violations = lintContent("const x: any = value as string; // tyforge-lint-disable-line");
    assert.equal(violations.length, 0);
  });

  it("disable-line with rule name disables only that rule", () => {
    const violations = lintContent("const x: any = value as string; // tyforge-lint-disable-line no-cast");
    assert.equal(violations.length, 1);
