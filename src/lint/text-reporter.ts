...
export class TextReporter implements IReporter {
  ...
  report(violations: IRuleViolation[], fixCount: number = 0): void {
    if (violations.length === 0 && fixCount === 0) {
      console.log(`${TextReporter.BOLD}tyforge-lint:${TextReporter.RESET} no violations found`);
      return;
    }
    ...
    console.log(`\n${TextReporter.BOLD}tyforge-lint:${TextReporter.RESET} ${TextReporter.RED}${errors} error(s)${TextReporter.RESET}, ${TextReporter.YELLOW}${warnings} warning(s)${TextReporter.RESET}`);
