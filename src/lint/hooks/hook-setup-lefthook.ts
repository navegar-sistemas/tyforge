  ...
  setup(command: string, cwd: string): IHookSetupResult {
    ...
    try {
      if (fs.existsSync(hookPath)) {
        ...
        if (existing.includes(HOOK_MARKER)) {
          return {
            success: true,
            hookPath,
            message: "Lefthook config already contains tyforge-lint",
            manualSteps: [],
          };
        }
      ...
      return {
        ...
        manualSteps: [
          `Manually add a tyforge-lint command to ${hookPath} under pre-commit`,
        ],
      };
    }
  }
    ...
    try {
      ...
      if (!content.includes(HOOK_MARKER)) {
        return {
          success: true,
          hookPath,
          message: "Lefthook config does not contain tyforge-lint",
          manualSteps: [],
        };
      }
      ...
      return {
        success: true,
        hookPath,
        message: "Lefthook tyforge-lint block removed",
        manualSteps: [],
      };
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : "Unknown error";
      return {
        ...
        manualSteps: [`Manually remove tyforge-lint block from ${hookPath}`],
      };
    }
  }
