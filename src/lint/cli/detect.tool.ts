...
export class ToolDetect {
  ...
  static hasExistingConfig(cwd: string): boolean {
    ...
    const unifiedPath = path.resolve(cwd, "tyforge.config.json");
    if (!fs.existsSync(unifiedPath)) return false;
    try {
      const content = fs.readFileSync(unifiedPath, "utf-8");
      const parsed: unknown = JSON.parse(content);
      return TypeGuard.isRecord(parsed) && TypeGuard.isRecord(parsed["lint"]);
    } catch {
      return false;
    }
  }
